...
def get_signature_prompt_template(_task):
    script_dir_path = os.path.dirname(os.getcwd())
    prompt_path = os.path.join(script_dir_path, 'Scripts/prompts/SIGNATURE_TEMPLATE.file')
    if not (os.path.exists(prompt_path)):
        print("Scripts/prompts/SIGNATURE_TEMPLATE.file not found!!")
        return
    ...
    final_prompt = template.format(task_description=_task['task_description'],
                                   ...
                                   test_1=_task['test_cases']['test_1'],
                                   test_2=_task['test_cases']['test_2'],
                                   test_3=_task['test_cases']['test_3'])
    # print(final_prompt)
    return final_prompt
...
def invoke_gpt4(_task, _temp, _key):
    openai.api_key = _key
    prompt_ = get_signature_prompt_template(_task)
...
def execute_signature_prompt(_api_config, _env_config):
    ...
    utility.save_to_json(all_response,
                         os.path.join(_env_config["base_output_path"],
                                      "rq2-signature-prompting-" + model + ".json"))
